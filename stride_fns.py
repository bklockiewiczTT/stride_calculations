...
def read_next_tile(
    ...
    # print(f"row_in_mm_M_block_idx: {row_in_mm_M_block_idx}, new_col: {new_col}, mm_M_block_idx: {mm_M_block_idx}")
...
def how_many_tiles_to_read_formula(
    ...
    current_block_tiles_remaining = chunk_piece_size - (row_in_mm_M_block_idx * chunk_width_in_tiles + chunk_col_in_tiles) - 1
    future_blocks_tiles = (last_mm_M_block_idx - mm_M_block_idx) * chunk_piece_size
...
def read_tiles_granular(
    ...
    if last_mm_M_block_idx < start_mm_M_block_idx:
        raise ValueError("last_mm_M_block_idx must be greater than or equal to start_mm_M_block_idx")

    first_row_in_mm_M_block_idx, first_chunk_col_in_tiles, first_mm_M_block_idx = read_next_tile(start_row_in_mm_M_block_idx, start_chunk_col_in_tiles, start_mm_M_block_idx, worker_id, chunk_piece_size, chunk_width_in_tiles, mm_block_unit_ht)
    if first_mm_M_block_idx > last_mm_M_block_idx:
        return
    tiles_to_read = how_many_tiles_to_read_formula(first_row_in_mm_M_block_idx, first_chunk_col_in_tiles, first_mm_M_block_idx, advance_by_tiles, last_mm_M_block_idx, chunk_piece_size, chunk_width_in_tiles)
    while tiles_to_read > 0:
        ...
        for i in range(tiles_to_read_in_this_step):
            read_tile_coords(i, first_row_in_mm_M_block_idx, first_chunk_col_in_tiles, first_mm_M_block_idx)
            first_row_in_mm_M_block_idx, first_chunk_col_in_tiles, first_mm_M_block_idx = read_next_tile(first_row_in_mm_M_block_idx, first_chunk_col_in_tiles, first_mm_M_block_idx, advance_by_tiles, chunk_piece_size, chunk_width_in_tiles, mm_block_unit_ht)
...
def read_tile_coords(
    ...
    Read the this tile.
    """
    print(f"i: {i}, row_in_mm_M_block_idx: {row_in_mm_M_block_idx}, chunk_col_in_tiles: {chunk_col_in_tiles}, mm_M_block_idx: {mm_M_block_idx}")
...
if __name__ == "__main__":
    ...
    read_tiles_granular(worker_id, row_in_mm_M_block_idx, chunk_col_in_tiles, mm_M_block_idx, advance_by_tiles, last_mm_M_block_idx, tile_granularity, chunk_piece_size, chunk_width_in_tiles, mm_block_unit_ht)
